...
class OrderServiceImplTest2 {
    private static final Logger log = LoggerFactory.getLogger(OrderServiceImplTest2.class);
    ...
    void createOrderConcurrencyTest(int concurrencyOrderUsers, int stock, long quantity)
        ...
        OrderCreateDto request = new OrderCreateDto(
                member.getId(),
                OrderCreateDto.orderCreateDtoProducts(saveProducts.stream().map(Product::getId).toList(), List.of(quantity, quantity, quantity))
        );
        ...
        for (int i = 0; i < concurrencyOrderUsers; i++) {
            executor.submit(() -> {
                try {
                    ...
                    orderService.create(request); //로직 실행(race-condition 발생)
        ...
        for (Long q : request.getProductCounts()) {
            expectedSuccess = Math.min(expectedSuccess, stock / q.intValue());
        }

        // 상품별 검증
        for (Product p : products) {
            Long orderQty = request.findByIdProductCount(p.getId());
